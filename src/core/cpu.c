...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
    ...
    INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, rv_halt(R(10))); // FIXME
    ...
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, vaddr_write_b(src1 + imm, BITS(src2, 31, 0)));
    ...
    INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, assert(0)); // TODO: Use proper handling (Raise an exception?)
    INSTPAT_END();
    // clang-format on
